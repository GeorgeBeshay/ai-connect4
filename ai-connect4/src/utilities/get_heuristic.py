    ...
    for r in range(ROW_COUNT):
        for c in range(COL_COUNT):
            if c < COL_COUNT - 3:
                ...
                if r < ROW_COUNT - 3:
                    # check diagonal and empty down
                    if state[r][c] == state[r + 1][c + 1] == player_piece and state[r + 2][c + 2] == state[r + 3][c + 3] == 0:
                        two_count += 1
...
def calculate_heuristic(state: List[List[int]], computer_piece: int, human_piece: int):
    ...
    heuristic = 100 * more_than_three_ai_score + 75 * three_ai_score + 50 * two_ai_score - (
